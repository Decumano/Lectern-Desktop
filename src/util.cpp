...
bool spawn_detached(const fs::path &executable,
                    const std::vector<std::string> &args)
{
    ...
    if (CreateProcessA(nullptr, buffer.data(), nullptr, nullptr, FALSE,
                       DETACHED_PROCESS, nullptr, nullptr, &startup,
                       &info) == 0)
    {
        return false;
    }
    // Closing both handles leaves the child running on its own.
    ...
    if (pid == 0)
    {
        // Double-fork so the installer is reparented to init and never
        // becomes a zombie once this process exits.
        if (fork() == 0)
        {
            setsid();
