// Small helpers shared across the app: hashing, ids, base64, time and the
...
/// Decodes lowercase or uppercase hex. Nullopt on odd length or a non-hex
/// character — used for keys and signatures, where a silent partial decode
...
/// Starts a program without waiting for it and without holding it open, so
/// the parent can exit immediately afterwards. Used to hand off to an
/// installer. Returns false if the process could not be started.
